# Ryan Brown Lab 6: ggplot

my.dir <- "/Users/broiwniemixxx/Desktop/IST719/Week 6/"
...
ggplot(sales) + 
  aes(x=expenses, y=income, color=rep.region, shape=type, alpha=unit.price, size=units.sold) + 
  geom_point()
...
ggplot(sales) + aes(x=expenses, y=income) + geom_point() + 
  geom_line(color="red", lwd=3) + aes(y=income.pred)

ggplot(sales) + aes(x=expenses, y=income) + geom_point(color="pink") +
  geom_rug() + geom_line(aes(y=income.pred)) + geom_line(aes(y=income.pred + 150)) +
  geom_vline(xintercept = 10, color="blue") + geom_hline(yintercept = 500, color="orange") +
...
ggplot(sales) + aes(x=expenses, y=income, color=price) + geom_freqpoly()
...
ggplot(sales) + aes(x=income) + 
  ...
  aes(y=..density..) + geom_density(alpha=0.3, fill="blue", color="blue")
...
ggplot(df) + aes(x=year, y=x) + 
  geom_ribbon(aes(ymin = df$x - 1000, ymax = df$x + 1000, fill="yellow")) +
...
ggplot(sales) + aes(x=income) +
  ...
  aes(y=..density..) + geom_density(fill="yellow", alpha=0.1)
